...
async function load() {
  loading.value = true;
  const { data } = await api.get<Paginated<Medicine>>("/medicines/", { params: { page_size: 200 } });
...
<template>
  <div class="space-y-4">
    ...
    <form class="grid gap-2 rounded-2xl border border-[var(--line)] bg-[var(--card)] p-3" @submit.prevent="save">
      <input v-model="form.name" required class="rounded-xl border border-[var(--line)] px-3 py-2" :placeholder="t('medicine.name')" />
      <input v-model="form.active_ingredient" class="rounded-xl border border-[var(--line)] px-3 py-2" :placeholder="t('medicine.ingredient')" />
      <input v-model="form.unit" class="rounded-xl border border-[var(--line)] px-3 py-2" :placeholder="t('medicine.unit')" />
      <div class="flex gap-2">
        <button class="rounded-xl bg-[var(--accent)] px-3 py-2 text-white">{{ t("common.save") }}</button>
        <button v-if="editing" type="button" class="rounded-xl border px-3 py-2" @click="editing = null; form = { name: '', active_ingredient: '', unit: 'cpr' }">
          {{ t("common.cancel") }}
        </button>
      </div>
    </form>
